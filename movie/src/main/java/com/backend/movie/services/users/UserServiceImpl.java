...
import com.backend.movie.helpers.mapper.UserMapper;
...
public class UserServiceImpl implements UserService{
    ...
    public User editUserProfile(UUID userId, EditProfileRequest request) throws BadRequestException {
        ...
        return mapper.map(saveUser(userEntity));
    }
    ...
    @Override
    public User save(RegisterRequest request) throws BadRequestException {
        if(existByLogin(request.getLogin())) {
            throw new BadRequestException("Логин занят");
        }
        UserEntity newUser = mapper.map(request);
